    ...
    if (!isLoaded || favorites.length === 0) {
        return (
            <div className="space-y-6">
                <h3 className="text-lg font-semibold text-white uppercase tracking-wider">Saved Plots</h3>
        ...
        <div className="space-y-6">
            ...
            <ul className="space-y-3 text-sm">
                {savedPlots.map((plot) => (
                    plot && (
                        <li key={plot.id}>
                            <Link href={`/plots/${plot.id}`} className="hover:text-gold-400 transition-colors flex items-center justify-between group">
                                <span className="truncate pr-2">{plot.title}</span>
                                <span className="text-xs text-gold-500/70 group-hover:text-gold-400 flex-shrink-0 whitespace-nowrap">{plot.price}</span>
                            </Link>
                        </li>
                    )
                ))}
